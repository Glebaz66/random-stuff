 ...
 */
const getUsersByEyeColor = (_arr, color) => users.filter(user => user.eyeColor === color);
 ...
 */
const getUsersByGender = (_arr, gender) => users.map(user => user.gender === gender);
 ...
 */
const getInactiveUsers = _arr => users.filter(user => user.isActive);
 ...
 */
const getUserByEmail = (_arr, email) => users.find(user => user.email === email);
 ...
 */
const getUsersWithAge = (_arr, min, max) => users.filter(user => user.age > min && user.age < max);
 ...
 */
const getTotalBalance = arr => users.reduce((acc, bal) => acc + bal.balance, 0);
 ...
 */
const getUsersByFriend = (arr, name) => users.filter(user => user.friends.includes(name));
...
*/
const getAllUniqueSkills = arr => users.reduce((acc, user) => acc.concat(user.skills), [])
                                        .reduce((acc, skill) => !acc.includes(skill) ? acc.concat(skill) : acc, [])
                                        .sort();

console.log(getAllUniqueSkills(users));// [ 'adipisicing', 'amet', 'anim', 'commodo', 'culpa', 'elit', 'ex', 'ipsum', 'irure', 'laborum', 'lorem', 'mollit', 'non', 'nostrud', 'nulla', 'proident', 'tempor', 'velit', 'veniam' ]

                             
/**
* Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (поле friends)
*/
const getUserNamesSortedByFriendsCount = arr => users.sort((a, b) => a.friends.length - b.friends.length).map(user => user.name)
// acc + user.friends.length, []).split('').sort((a, b) => b - a) ;

console.log(getUserNamesSortedByFriendsCount('users: ', typeof users)); 
